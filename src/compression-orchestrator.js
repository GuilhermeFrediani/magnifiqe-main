 ...
 *   6. Semantic compression tiers remove predictable grammar words
 */
...
export function compressToolOutput(text, opts = {}) {
  ...
  try {
    // Step 4.5: Apply CCR compression
    const ccrResult = compress(processedText, { forceLevel: level, contentType });
    let compressed = ccrResult.compressed;

    // Step 4.5: Semantic compression tiers for text content
    // Only applies to prose-like content (text/plain), not structured formats
    if (contentType === "text" && compressed.length > 100) {
      const semanticTier = level >= CompressionLevel.SEMANTIC ? 3 : level >= CompressionLevel.STRUCTURAL ? 2 : 1;
      const semanticResult = semanticCompress(compressed, { tier: semanticTier, preserveCode: true });
      if (semanticResult.compressed.length < compressed.length) {
  ...
  server.tool(
    ...
    async ({ text, tool_name, target_tokens }) => {
      try {
        ...
        return { content: [{ type: "text", text: `HALT — compress_tool_output failed: ${e.message}` }] };
      }
    }
  );
  ...
  server.tool(
    ...
    async ({ hash, query }) => {
      try {
        ...
        return { content: [{ type: "text", text: `HALT — decompress_tool_output failed: ${e.message}` }] };
      }
    }
  );
