...
- Navegação por Teclado: **NEVER** remova os `outlines` de focus de links ou botões sem fornecer uma estilização equivalente visível no estado `:focus-visible`.
- Botões e Interações: Ações de clique devem sempre ser feitas com a tag `<button>` ou `<a href>`, **NEVER** adicione `onClick` solto em uma `<div>`.
...
- **Flexbox e Grid (Layout 1D e 2D):** A abordagem primária para organizar layouts complexos. **AVOID** ao máximo o uso de floats ou margens e paddings hardcoded exagerados (ex: `margin-left: 200px`) para empurrar o conteúdo, a menos que absolutamente necessário.
...
- **Semântica no Estilo:** **NEVER** misture lógica de negócios com convenções de estilo na nomenclatura das classes (CSS Modules ou classes genéricas).
...
- O Frontend **NEVER** pode quebrar os padrões de "Largest Contentful Paint" com imagens gigantes não redimensionadas, ou "Cumulative Layout Shift" injetando blocos no topo sem altura previamente definida no CSS.
