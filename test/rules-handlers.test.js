...
describe('list_rules', () => {
  ...
  it('should list all 12 bundled rule files', async () => {
    const result = await tools.list_rules.handler({});
    const text = result.content[0].text;

    // All 12 numbered rule files should be present (00-11, council rules removed)
    for (let i = 0; i <= 11; i++) {
  ...
  it('should use stable alphabetical sort order', async () => {
    ...
    const idx13 = text.indexOf('14-');
    assert.ok(idx00 >= 0, '00-project-overview should be listed');
    assert.ok(idx13 >= 0, '14-modular-frontend should be listed');
    assert.ok(idx00 < idx13, '00- should appear before 14- (alphabetical order)');
  });
});
...
describe('get_rules_bundle', () => {
  ...
  it('should contain all 12 rule files in full mode', async () => {
    const result = await tools.get_rules_bundle.handler({ mode: 'full' });
    const text = result.content[0].text;
