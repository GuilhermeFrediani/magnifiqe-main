...
## Summary
Stack Perfeita MCP — Verification tools File/symbol verification, hallucination detection, groundedness scoring, and incremental diff tracking.
    ...
    async ({ claims, project_root }) => {
      ...
      try {
        ...
        for (const claim of claims) {
          ...
          let absPath;
          try {
            absPath = resolve(root, filePath);
          } catch {
            results.push({ claim: trimmed, status: "FAIL", reason: "Invalid path" });
            continue;
          }
          ...
          if (exportName) {
            ...
            const exportPatterns = [
              new RegExp(`export\\s+(default\\s+)?(async\\s+)?function\\s+${exportName}\\b`),
              new RegExp(`export\\s+(default\\s+)?(async\\s+)?const\\s+${exportName}\\b`),
              new RegExp(`export\\s+(default\\s+)?class\\s+${exportName}\\b`),
              new RegExp(`export\\s+\\{[^}]*\\b${exportName}\\b[^}]*\\}`),
              new RegExp(`module\\.exports\\s*=.*\\b${exportName}\\b`),
  ...
  server.tool(
    ...
    async ({ file_path, last_hash }) => {
      ...
      try {
        const absPath = validateAbsolutePath(file_path);
        if (!existsSync(absPath)) {
          return { content: [{ type: "text", text: `FAIL — File not found: ${absPath}` }] };
        }
