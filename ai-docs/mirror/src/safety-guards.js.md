 ...
 * - Session TTL: auto-expire old council sessions
...
/**
 * Filter council sessions, removing those older than TTL.
 * @param {object[]} sessions - Array of council sessions
 * @returns {{ kept: object[], expired: object[], expiredCount: number }}
 */
export function filterExpiredSessions(sessions) {
  if (!Array.isArray(sessions)) return { kept: [], expired: [], expiredCount: 0 };
  ...
  return { kept, expired, expiredCount: expired.length };
}
