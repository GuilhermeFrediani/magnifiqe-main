...
## Summary
Stack Perfeita MCP — TTSR Engine (Time-Traveling Streamed Rules) Monitors LLM output against registered regex/AST conditions. When a rule fires: abort → inject corrective reminder → retry. Port of OMP's TtsrManager pattern adapted for MCP server context.
